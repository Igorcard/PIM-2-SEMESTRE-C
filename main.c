...
typedef struct
{ // ESTRUTURA PARA CADASTRO DE FUNCIONARIO;
...
int TelaLogin()
{ // TELA DE LOGIN PARA ACESSAR O PROGRAMA;
    char senha[12], usuario[12];
    int *user; // VETOR PARA DETERMINAR O DOMINIO
    user = malloc(sizeof(int));
    printf("================TELA DE LOGIN================\n");
    do
    { // ESTRUTURA DE REPETICAOO ATE ACERTAR A SENHA E O USUARIO;
        printf("USUARIO: ");
        gets(usuario);
        printf("SENHA: ");
        gets(senha);
...
int MenuInicial(int user)
{ // TELA DE MENU INICIAL PARA SELECIONAR A EXECUCAOO "user" DETERMINA QUAIS OPCOES TERA PELO SEU DOMINIO;
    int opcao;
    ...
    { // ESTRUTURA DE REPETICAOO PARA ACEITAR APENAS AS OPCOES DISPONIVEIS;
        printf("\n=========OPCAOO INVALIDA===========\n");
        printf("OPCAO: ");
        scanf("%d", &opcao);
    }
    do
    { // ESTRUTURA DE REPETICAOO PARA INIBIR ACESSO A FUNCOES FORA DE SEU DOMINIO;
...
int CadastroF(CadFuncionario **f, int quant, int tam)
{ // TELA DE CADASTROS PARA ADICIONAR FUNCIONARIOS E CLIENTES
    int escolha;
    ...
    if (quant < tam)
    {
        ...
        { // ESTRUTURA DE REPETICAO PARA ACEITAR SOMENTE AS OPCOES DISPONIVEIS;
...
int LerF(CadFuncionario **f)
{
    ...
    { // ESTRUTURA DE CONDICAO PARA CASO DE ERRO EM ABRIR O ARQUIVO;
        printf("\n!!!!!ERRO AO ABRIR ARQUIVO!!!!!\n");
    }
    fclose(pont_arq);
}
...
int CadastroC(CadCliente **c, int quant, int tam)
{ // TELA DE CADASTROS PARA ADICIONAR CLIENTES
    int escolha;
    float plan;
